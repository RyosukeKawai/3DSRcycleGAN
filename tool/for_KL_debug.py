...
def KLD(lr_hist, hr_hist, epsilon):
    ...
    hi_sum=np.sum([li * np.log(li / hi) for li, hi in zip(lr_hist, hr_hist)])
    return (li_sum+hi_sum)*0.5
        ...
        for e in range(len(epshiron)):
                ...
                for num_b in range(len(num_bins)):
                        ...
                        for i in range(loop):
                                ...
                                #calc num of bins
                                binx = int(float(abs(int(np.min(x3) - 1)) + int(np.max(x3) + 1)) * num_bin+0.5)
                                biny = int(float(abs(int(np.min(y3) - 1)) + int(np.max(y3) + 1)) * num_bin+0.5)
                                x_bin[i]=binx
                                y_bin[i]=biny
                        ...
                        df.to_csv('{}/results_double_KLD_another1.csv'.format(result_dir), index=False, encoding='utf-8', mode='a',header=None)
